...
let allCaps = [];
// can I do this next line?
// allCaps = runners.map(x => x.toUpperCase());
// console.log(allCaps); 

runners.forEach(function(entry) {
    allCaps.push(`${entry.first_name}`);
});
...
function checkLarge(entry) {
    return entry.shirt_size == "L";
}

largeShirts = runners.filter(checkLarge);
...
runners.forEach(function(entry) {
    ticketPriceTotal.push(entry.donation)
})
const reducer = (accumulator, currentValue) => accumulator + currentValue;

console.log(ticketPriceTotal.reduce(reducer, 0));

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - get all emails
...
// Problem 2  one company is generous, they will double each individual donation but want to add it to each person's donation
// return a list of the new, doubled donations
...
let large_donor_emails = [];

function checkDonationSize(entry) {
    return entry.donation > 100;
}
...
largeDonors.forEach(function(entry) {
    largeDonorEmails.push(`${entry.email}`);
})
console.log(largeDonorEmails);